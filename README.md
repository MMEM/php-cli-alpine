...
Creating a new build with tag '8.2' without multi-arch support:

    $ docker build -t php-cli-alpine:8.2 -f ./Dockerfile .
    $ docker tag php-cli-alpine:8.2 metalmanufactures/php-cli-alpine:8.2
    $ docker login 
    $ docker push metalmanufactures/php-cli-alpine:8.2
...
Sometimes the base php image bakes in libraries in minor versions. For example the tokenizer library. You can run this to check that:
    ...
    $ docker run --rm php:8.2-cli-alpine php -i | grep -i token

Creating a new build with tag '8.2' **with** multi-arch support:

    $ docker buildx build --platform linux/arm64,linux/amd64 --push -t metalmanufactures/php-cli-alpine:8.2 -f ./Dockerfile .
